# Erreur #003 : PasswordsController POST #create (sans token Turnstile)
...
- **Ligne** : 93
- **Test** : `bloque la demande de réinitialisation` (sans token Turnstile)
- **Commande pour reproduire** :
  ```bash
  docker exec grenoble-roller-dev bundle exec rspec ./spec/controllers/passwords_controller_spec.rb:93
  ```
...
# ./spec/controllers/passwords_controller_spec.rb:94:in 'block (4 levels) in <top (required)>'

Finished in 1.18 seconds (files took 1.63 seconds to load)
1 example, 1 failure
```
